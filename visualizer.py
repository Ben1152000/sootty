...
class Visualizer:
    ...
    @staticmethod
    def wiretrace_to_svg(wiretrace, start=0, length=1):
        width = 2 * Visualizer.LEFT_MARGIN + (length + 1) * Visualizer.DATA_WIDTH
        height = 2 * Visualizer.TOP_MARGIN + sum([sum([1 for wire in wiregroup.wires]) for wiregroup in wiretrace.wiregroups]) * (Visualizer.WIRE_HEIGHT + Visualizer.WIRE_MARGIN) - Visualizer.WIRE_MARGIN
    ...
    @staticmethod
    def wiregroup_to_svg(wiregroup, left, top, start, length):
        svg = ''
    ...
    def wire_to_svg(wire, left, top, start, length):
        ...
        for index in range(start, start + length):
            ...
            svg += Visualizer.value_to_svg(
                ...
                left = left + ((index - start + 1) * Visualizer.DATA_WIDTH),
                top = top,
                initial = (index == start))
        return svg
